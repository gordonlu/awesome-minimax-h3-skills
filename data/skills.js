...
window.AMHS_DATA = {
  ...
  skills: [
     ...
     * ---------------------------------------------------------- */
      ...
      version: "0.1.0",
      summary: {
        en: "Exactly two images — a start frame and an end frame — with a continuous, progressively narrowing motion path between them.",
        zh: "恰好两张图（首帧 + 尾帧），由正文补上一条连续、逐帧收窄的转换路径。",
      },
      description: {
        en: "For from-A-to-B transitions with exactly two reference images: pose-to-pose, outfit changes, object-state changes, scene or lighting transitions, growth or decay, assembly or deconstruction. Picture 1 anchors the opening, Picture 2 anchors the ending; the prompt body supplies the motion path — never two static image descriptions — following base-en.txt exactly: verbatim alignment line, single-shot preference, and the first-frame state → observable intermediate changes → progressively narrowing differences → last-frame state structure. Includes a canonical dress-change demo and the official umbrella-woman case verbatim.",
        zh: "专为「恰好两张参考图」的 A→B 转换需求：姿态转换、换装、物体状态变化、场景或光线过渡、生长与衰变、组装与拆解。Picture 1 锚定开场、Picture 2 锚定结尾；提示词正文只写连接两帧的运动路径——绝不重复两段静态描述——严格遵循 base-en.txt：逐字对齐行、单镜头优先、首帧状态→可观察的中间变化→差异逐帧收窄→尾帧状态。附带变装 canonical demo 与官方雨伞女郎原文案例。",
      },
      categories: ["animation", "creative"],
      tags: [
        { en: "FL2VA", zh: "FL2VA" },
        { en: "First-Last Frame", zh: "首尾帧" },
        { en: "Transition", zh: "转换" },
        { en: "Before/After", zh: "前后对比" },
      ],
      languages: ["en", "zh"],
      preview: {
        poster: "community-skills/h3-fl2va-film/assets/poster.webp",
        video: "",
        sourceUrl: "",
        caption: { en: "Placeholder poster — real output clip pending (generate with H3 via the skill)", zh: "占位海报 — 实拍成片待生成（用本 Skill 在 H3 生成）" },
      },
      inputs: [
        { en: "Exactly two images: Picture 1 (start state) and Picture 2 (end state)", zh: "恰好两张图：Picture 1（起始状态）与 Picture 2（结束状态）" },
        { en: "What transitions: pose, outfit, object state, scene, lighting", zh: "转换什么：姿态、造型、物体状态、场景、光线" },
        { en: "Duration (default 8s), aspect ratio, single shot vs explicit multi-shot", zh: "时长（默认 8s）、画幅、单镜头或多镜头" },
      ],
      capabilities: [
        { en: "Writes the verbatim FL2VA alignment line with a correct S.SS end timestamp", zh: "撰写逐字 FL2VA 对齐行，S.SS 结束时间戳精确" },
        { en: "Supplies a motion path, never two static image descriptions", zh: "正文只写运动路径，不重复两段静态描述" },
        { en: "Narrows differences progressively: first-frame state → intermediates → last-frame state", zh: "差异逐帧收窄：首帧状态 → 中间变化 → 尾帧状态" },
        { en: "Keeps identity anchors alive across the transformation", zh: "身份锚点在转换过程中全程存活" },
      ],
      workflow: [
        { id: "intake", title: { en: "Confirm the two anchors", zh: "确认两个锚点" }, desc: { en: "Both images, which is Picture 1 / Picture 2, and what transitions.", zh: "两张图、谁是 1 谁是 2、转换什么。" } },
        { id: "align", title: { en: "Pin the alignment line", zh: "钉死对齐行" }, desc: { en: "Copied verbatim; S.SS equals the exact duration.", zh: "逐字复制；S.SS 等于精确时长。" } },
        { id: "path", title: { en: "Write the motion path", zh: "写运动路径" }, desc: { en: "First-frame state → observable changes → narrowing differences → last-frame state; single shot.", zh: "首帧状态 → 可观察变化 → 差异收窄 → 尾帧状态；单镜头。" } },
        { id: "qc", title: { en: "Generate and QC", zh: "生成并验收" }, desc: { en: "Opening equals Picture 1, final frame equals Picture 2, no jumps, identity holds.", zh: "开场等同 Picture 1、末帧等同 Picture 2、无跳变、身份一致。" } },
      ],
      outputs: [
        { en: "A transition video (MP4) opening on Picture 1 and landing on Picture 2", zh: "一段开场=图1、结尾=图2 的转换成片（MP4）" },
        { en: "The FL2VA prompt with the verbatim alignment line, ready to reuse", zh: "含逐字对齐行的 FL2VA 提示词，可直接复用" },
      ],
      modes: [
        { id: "FL2VA", en: "Primary and only mode: exactly two images, start + end frame.", zh: "主模式（唯一）：恰好两张图，首帧 + 尾帧。" },
      ],
      promptStructures: [
        {
          label: { en: "FL2VA two-image transition", zh: "FL2VA 双图转换" },
          fields: ["alignment line (verbatim)", "integrated_multimodal_description", "overall_soundscape", "non_diegetic_music"],
        },
      ],
      bestFor: [
        { en: "Before/after transitions with exactly two images", zh: "恰好两张图的前后对比转换" },
        { en: "Pose-to-pose and outfit-change transitions", zh: "姿态转换与换装" },
        { en: "Object-state, scene, lighting, growth/decay changes", zh: "物体状态、场景、光线、生长/衰变变化" },
      ],
      notFor: [
        { en: "One image only (I2VA / L2VA)", zh: "只有一张图（I2VA / L2VA）" },
        { en: "More than two keyframes (multi-keyframe montage)", zh: "超过两张关键帧（多关键帧蒙太奇）" },
        { en: "Pure-text requests without any image", zh: "无图的纯文本需求" },
      ],
      install: {
        command: "npx skills add https://github.com/gordonlu/awesome-minimax-h3-skills --skill h3-fl2va-film",
      },
      sources: {
        repository: "https://github.com/gordonlu/awesome-minimax-h3-skills",
        skillDir: "https://github.com/gordonlu/awesome-minimax-h3-skills/tree/main/community-skills/h3-fl2va-film",
        skillMd: "https://github.com/gordonlu/awesome-minimax-h3-skills/blob/main/community-skills/h3-fl2va-film/SKILL.md",
        skillCnMd: "",
        docs: [
          { label: { en: "prompt-library.md — 3 runnable FL2VA cases", zh: "prompt-library.md — 3 个可运行 FL2VA 案例" }, url: "https://github.com/gordonlu/awesome-minimax-h3-skills/blob/main/community-skills/h3-fl2va-film/references/prompt-library.md" },
        ],
      },
    },
